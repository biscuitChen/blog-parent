...
import com.serein.blog.utils.JWTUtils;
...
public class LoginServiceImpl implements LoginService {
    ...
    private static final String slat = "serein!@#";
    ...
    public Result login(LoginParam loginParam) {
        /**
         * 1、检查参数是否合法
         * 2、根据用户名和密码去user表中给查询是否存在
         * 3、如果不存在，则登录失败
         * 4、如果存在，使用jwt生成token，返回给前端
         * 5、token放入redis中，redis    token：user
         * 信息设置过期时间（登录认证的时候，先认证token字符串是否普合法，去redis认证是否存在）
         */
